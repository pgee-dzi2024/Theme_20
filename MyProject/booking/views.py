...
def show_select(request):
    if(request.method == "GET" and len(request.GET)!=0):
        ...
        shows = show.objects.filter(end_date__gte=date,
                                    start_date__lte=date).select_related('movie_id', 'movie__photo', 'movie__movie_name').order_by('movie_id', 'showtime').values_list('id','price','showtime','movie','movie__photo','movie__movie_name',named=True)
        res_dict = {}
...
def userbookings(request):
    ...
    booking_table = booking.objects.filter(user=request.user).select_related().order_by('-booked_date')
    # .values_list('id','show_date','booked_date','show__movie__movie_name','photo','show__showtime','total','seat_num',named=True))
